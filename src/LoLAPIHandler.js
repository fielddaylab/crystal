//Contains all functions for interacting with the LoL API

//Send messages
function LoLApi (messageName, payloadObj) {
    let msg={
        "message": messageName,
        "payload": JSON.stringify(payloadObj)
    };

    parent.postMessage(msg,'*');
}

//Called when the game starts, tells LoL API game is ready
function gameStart()
{
    LoLApi("gameIsReady", 
    { 
        aspectRatio: "16:9",
        resolution: "1024x660",
    });

	console.log("gameIsReady sent"); 
}

//Listen for messages
window.addEventListener("message", function (msg) {
    //console.log('[PARENT => UNITY]', msg);
    const {messageName, payload} = msg.data; 
    var prevState;

    switch (messageName) 
    {
        case 'pause':
            gameIsPaused = true;
            break;
        case 'resume':
            gameIsPaused = false;
            break;
    }
});

var gameIsPaused = false;
var sentComplete = false;

//Progress Handling
  var sentProgress = false;
  var currProgress = 0;
  var currScore = 0;
  var maxProgress = 9;
  var progArray = new Array(maxProgress).fill(0);
  var scoreArray = new Array(maxProgress).fill(0);

  //Add tutorial progress
  function completeLevel(index, score)
  {
    var update = false;

    dScore = score - scoreArray[index]
    if (dScore > 0)
    {
      scoreArray[index] = score
      update = true;
    }

    if (progArray[index] == 0) //first time level is being completed
    {
      progArray[index] = 1;
      currProgress = Math.min(maxProgress, currProgress + 1);
      update = true;
    }

    if (update)
    {
      sendProgress();
      checkGameEnd();
    }
  }

  //Send new progress to LoL API
  function sendProgress()
  {
    LoLApi('progress', {score: currScore, currentProgress: currProgress, maximumProgress: maxProgress});

    console.log("progress: " + currProgress + "/"+maxProgress);
  }

//Determine if progress points are maxed. If so, end the game
function checkGameEnd()
{
  if (currProgress >= maxProgress)
  {
    endGame();
  }
}

//Tell LoL API to end the game
function endGame()
{
  if (!sentComplete)
  {
    sentComplete = true;
    LoLApi('complete', {});
    console.log("Complete");
  }
}
