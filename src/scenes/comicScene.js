var ComicScene = function (game, stage) {
	...
	self.ready = function (args) {
			...
			if (i - args.start == 0) {
				imgs[0].onload = function () {
					...
					for (var i = 0; i < slots.length; i++) {
						slots[i].x =
							(slots[i].x - old_x) * scaling_ratio + old_x;
		...
		full_btn = new ButtonBox(0, 0, dc.width, dc.height, function (evt) {
			if (!hit_ui) next_btn.click(evt);
		});
		nodes = [];
		for (var i = 0; i < imgs.length; i++) {
			(function (i) {
				var x = (i + 10) * (dc.width / (imgs.length + 19));
				nodes[i] = new ButtonBox(
					x - node_s / 2,
					node_y - btn_s / 2,
					btn_s,
					btn_s,
	...
	self.tick = function () {
		if (cur_img >= imgs.length) {
			...
			clicker.filter(prev_btn);
			clicker.filter(next_btn);
			clicker.filter(skip_btn);
			clicker.filter(full_btn);
			clicker.flush();
		}
	...
	self.draw = function () {
		...
		if (cur_img < imgs.length) {
			...
			if (cur_img - 2 >= 0) {
				slot = slots[0];
				lerp_slot = slot;
				if (delta_goal < 0) lerp_slot = slots[1];
				drawImageCenteredWithin(
					imgs[cur_img - 2],
					lerp(slot.x, lerp_slot.x, abs(delta)),
					lerp(slot.y, lerp_slot.y, abs(delta)),
					lerp(slot.w, lerp_slot.w, abs(delta)),
					lerp(slot.h, lerp_slot.h, abs(delta))
				);
			}
			if (cur_img + 2 < imgs.length) {
				slot = slots[4];
				lerp_slot = slot;
				if (delta_goal > 0) lerp_slot = slots[3];
				drawImageCenteredWithin(
					imgs[cur_img + 2],
					lerp(slot.x, lerp_slot.x, abs(delta)),
					lerp(slot.y, lerp_slot.y, abs(delta)),
			...
			if (delta_goal == 0) {
				...
				if (cur_img >= 0 && cur_img < imgs.length) {
					slot = slots[2];
					lerp_slot = slot;
					drawImageCenteredWithin(
						imgs[cur_img],
			...
			ctx.fillText(
				"SKIP",
				skip_btn.x + skip_btn.w - 2,
				skip_btn.y + skip_btn.h - 2
			);
			var x;
			for (var i = 0; i < imgs.length; i++) {
				ctx.fillStyle = blue;
				ctx.beginPath();
				ctx.arc(
					nodes[i].x + nodes[i].w / 2,
					nodes[i].y + nodes[i].h / 2,
					node_s / 2,
					0,
					2 * Math.PI
				);
			...
			var lerp_node = node;
			if (delta_goal < 0 && cur_img - 1 >= 0)
				lerp_node = nodes[cur_img - 1];
			if (delta_goal > 0 && cur_img + 1 < imgs.length)
				lerp_node = nodes[cur_img + 1];
			ctx.fillStyle = blue;
			ctx.beginPath();
			ctx.arc(
				lerp(node.x, lerp_node.x, abs(delta)) + node.w / 2,
				node.y + node.h / 2,
				node_s / 3,
				0,
				2 * Math.PI
